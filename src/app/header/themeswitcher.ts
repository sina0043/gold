...
@Component({
    ...
    template: `
    <button type="button" class="inline-flex w-8 h-8 items-center justify-center border border-surface-200 dark:border-surface-600 rounded" (click)="onThemeToggler()">
...
@Component({
    ...
    template: ` <div class="card flex justify-end">
        <ul class="flex list-none m-0 p-0 gap-2 items-center">
            <li>
              <div pRipple class="ripple-box">
                <button aria-label="mode" type="button" class="inline-flex w-8 h-8 p-0 items-center justify-center surface-0 dark:surface-800 border border-surface-200 dark:border-surface-600 rounded" (click)="onThemeToggler()">
                    <i [ngClass]="'pi ' + iconClass()"></i>
                </button>
              </div>
            </li>
            <li class="relative">
              <button
                  ...
                  type="button"
                  class="ripple-box inline-flex w-8 h-8 p-0 items-center justify-center surface-0 dark:surface-800 border border-surface-200 dark:border-surface-600 rounded"
              >
                  <i class="pi pi-palette"></i>
              </button>
              <div class="absolute top-[2.5rem] right-[-147px] hidden w-[18rem] p-3 bg-white dark:bg-surface-800 rounded-md shadow border border-surface-200 dark:border-surface-700 flex-col justify-start items-start gap-3.5 origin-top z-10">
                  <div class="flex-col justify-start items-start gap-2 inline-flex pr-2">
                      <span class="text-sm font-medium">رنگ دکمه ها و انیمیشن ها</span>
                  ...
                  <div class="flex-col justify-start items-start gap-2 inline-flex pr-2">
                      <span class="text-sm font-medium">رنگ کارد ها</span>
...
export class ThemeSwitcher {
  ...
  getPresetExt() {
    const color = this.primaryColors().find(
      (c: any) => c.name === this.selectedPrimaryColor()
    );
  ...
  savethemeState(state: any): void {
    if (isPlatformBrowser(this.platformId)) {
      localStorage.setItem(this.STORAGE_KEY, JSON.stringify(state));
      this.shareDataService.changeThemeColor$.next();
    }
  }
}